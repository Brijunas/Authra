---
name: new-endpoint
description: "Quick workflow for adding a single REST endpoint to an existing feature — DTOs, validator, service method, endpoint registration, and integration test."
when_to_use: "When the user runs `/new-endpoint` or explicitly asks to add a new endpoint to an existing feature."
argument-hint: "\"<METHOD> /v1/{path}\" [--feature <FeatureName>] [--permission <permission>]"
version: 2.2.0
---
...
```
/new-endpoint "<METHOD> /v1/{path}" [--feature <FeatureName>] [--permission <permission>]
```
...
| `security-practices` | Permission checking |
| `csharp-conventions` | C# coding standards |
| `testing-conventions` | Integration test patterns |
...
| `csharp-coder` | Implement endpoint |
| `security-analyzer` | Validate permission requirements |
| `tester` | Create integration test |
...
```bash
/new-endpoint "POST /v1/tenants/{tenantId}/members/invite" --permission "accounts:invite"
```
