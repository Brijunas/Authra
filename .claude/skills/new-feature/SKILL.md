---
name: new-feature
description: "End-to-end workflow for a new Authra feature — Domain → Application → Infrastructure → API → Tests. Scaffolds service, DTOs, validators, endpoints, and tests across all Clean Architecture layers."
when_to_use: "When the user runs `/new-feature` or asks to create a full feature implementation end-to-end."
argument-hint: "<FeatureName> [--entity <EntityName>] [--endpoints <list>]"
version: 2.2.0
---
...
```
/new-feature <FeatureName> [--entity <EntityName>] [--endpoints <list>]
```
...
| Skill | Usage |
|-------|-------|
| `architecture-conventions` | Layer boundaries, feature folders, DI |
| `api-conventions` | REST patterns, validation, error handling |
| `csharp-conventions` | C# 14 coding standards |
| `security-practices` | Permission checks if applicable |
| `testing-conventions` | Test structure and patterns |
...
| `csharp-coder` | Implement Application and Infrastructure |
| `tester` | Create unit and integration tests |
...
```bash
/new-feature Tenants --entity Tenant
```
