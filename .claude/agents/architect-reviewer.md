...
- Work with `security-reviewer` on security architecture
- Guide `performance-reviewer` on performance design
- Assist `dotnet-developer` on service design and Clean Architecture
- Support `ef-core-specialist` on data architecture and RLS patterns
- Partner with `api-designer` on API architecture
- Coordinate with `test-engineer` on testing strategy
