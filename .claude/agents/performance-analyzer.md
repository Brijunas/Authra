...
model: opus
when_to_use: "When investigating N+1 queries, missing indexes, async bottlenecks, or benchmarking hot paths in the Authra identity flows."
memory: project
color: orange
version: 2.2.0
---
...
- Work with `persistence-specialist` on query optimization
- Coordinate with `software-architect` on caching architecture
- Support `csharp-coder` with performance patterns
